...
const vector<s485>& v = s2110->s320();
for( size_t n = 0; n < v.size(); ++n ) {
s371* ref = (s371*)v[n]->s335();
ref->s500( this, proc );
s483* s2109 = ref->s377();
s631.push_back( *ref );
s630.push_back( s2109->s335() );
s628.s541( ref->s617(), s630.back().s15<s484*>() );}}
s493 = 0; // s1152;
s591::s500( this, /*true*/ proc ); // ???DEBUG??? 220113
if( s1903.empty() ) {
